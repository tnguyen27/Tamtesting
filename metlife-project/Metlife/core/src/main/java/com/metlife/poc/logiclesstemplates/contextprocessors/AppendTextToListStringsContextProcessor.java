...
public class AppendTextToListStringsContextProcessor extends
    ...
    public final void process(
        ...
        if (contentModel.has(Constants.CONFIG_PROPERTIES_KEY + Constants.DOT
            ...
            final boolean multi = propertyPath.contains(MetLifeConstants.COLON);

            String arrayProperty = null;

            if (multi) {
                arrayProperty = propertyPath.substring(0, propertyPath
                        .indexOf(MetLifeConstants.COLON));

                propertyPath = propertyPath.substring(propertyPath.indexOf(MetLifeConstants.COLON)
            ...
            } else if (contentModel.has((propertyPath))) {
                final ArrayList<String> propertyArray =
                        (ArrayList<String>) Utils.getPropertyAsList(contentModel, propertyPath);

                for (String element : propertyArray) {

                    final String path = element + tagListPropertyName;
                    propertyPath = propertyPath.substring(propertyPath.indexOf(Constants.DOT) + 1);

                    contentModel.set(Constants.CONTENT + Constants.DOT + propertyPath, path);
